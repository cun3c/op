...
print(f"Fetching SOL 4h...")
all_ohlcv = []
current_since = since
while True:
    try:
        ohlcv = exchange.fetch_ohlcv('SOL/USDT', '4h', since=current_since, limit=1000)
        if not ohlcv: break
...
df_4h = pd.DataFrame(all_ohlcv, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
        ...
        for r_ob in [70, 80]:
            for tp in [0.03, 0.05, 0.1]:
                for sl in [0.05, 0.10, 0.15]:
                    t, wr, pnl = run_test(df_4h, r_len, r_os, r_ob, tp, sl)
...
if bp:
    print(f"FOUND 4H: RSI={bp[0]}, OS={bp[1]}, OB={bp[2]}, TP={bp[3]*100}%, SL={bp[4]*100}% -> WR: {bp[6]*100:.2f}%, PnL: {bp[7]:.2f}%")
else:
